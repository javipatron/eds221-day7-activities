...
```{r}
bird_observation <- read_csv(here::here("data","52_pp52_birds_1.csv"))
...
birds_left_prof <- left_join(birds_subset, taxalist, by = "species_id")
```
...
birds_date <- birds_filter |> 
  mutate(surveymonth = lubridate::month(survey_date)) |> 
  mutate(surveymonth = as.factor(surveymonth))


class(birds_date$surveymonth)
...
-   Learn a new function on your own! Use `dplyr::relocate()` to move the new `survey_month` column to immediately after the `survey_date` column. You can do this in a separate code chunk, or pipe straight into it from your existing code.
    ...
    birds_date_relocate <- birds_date |> 
      relocate(surveymonth, .after = survey_date)
    ```
...
-   Find the total number of birds observed by *park* and *month* (i.e., you\'ll `group_by(park_name, survey_month)`)
    ...
    tot_birds <- birds_date_relocate |> 
      group_by(park_name, surveymonth) |> 
